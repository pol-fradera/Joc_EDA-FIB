          ...
          if (not cela_virus(u)) {
            if (not entra_ciutat_cami(u)) {
              if (p0.i <= rows()/2 and p0.j <= cols()/2) {
                if (random(0,1)) {
                  if (c2.unit_id == -1 and c2.type != WALL) move(id,BOTTOM);
                  else if (c3.unit_id == -1 and c3.type != WALL) move(id,RIGHT);
                  else if (c1.unit_id == -1 and c1.type != WALL) move(id,TOP);
                  else if (c4.unit_id == -1 and c4.type != WALL) move(id,LEFT);
                }
                else {
                  if (c3.unit_id == -1 and c3.type != WALL) move(id,RIGHT);
                ...
                if (random(0,1)) {
                  ...
                  if (c4.unit_id == -1 and c4.type != WALL) move(id,LEFT);
                  else if (c2.unit_id == -1 and c2.type != WALL) move(id,BOTTOM);
                  else if (c3.unit_id == -1 and c3.type != WALL) move(id,RIGHT);
                  else if (c1.unit_id == -1 and c1.type != WALL) move(id,TOP);
                }
