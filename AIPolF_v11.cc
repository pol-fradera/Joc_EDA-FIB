...
struct PLAYER_NAME : public Player {
  ...
  Pos ataca_bfs(const VVC& mapa, Pos origen, int health) {
    queue<Pos> q;
    map<Pos,Pos> m;
    bool meva_unitat = false;
    q.push(origen);
    m[origen] = origen;
    Cell orig = cell(origen);
    while (not q.empty()) {
      Pos actual = q.front();
      Cell c = cell(actual);
      q.pop();
      if (c.unit_id != -1) {
        ...
        if (u.player != me() and health > u.health) {
          Pos dest = actual;
          while (m[dest] != origen) {
            dest = m[dest];
          }
      ...
      for (int i = 0; i < 4; ++i) {
        Pos u = actual+Dir(i);
        Cell c1 = cell(u);
        if (c1.type == orig.type and m.find(u) == m.end()) {
          q.push(u);
          m[u] = actual;
        }
      }
  ...
  void move_units() {
    ...
    for (int i = 0; i < n; ++i) {
      ...
      //Pos p0 = u.pos;
      if (not ataca(u)) {
        if (not u.mask and not mascareta(u)) {
        //if (c.type == CITY or c.type == PATH) move(id, NONE);
          //if (not cela_virus(u)) {
          Cell c = cell(u.pos);
          Pos p1;
          if ((c.type == CITY and city_owner(c.city_id) != me()) or (c.type == PATH and path_owner(c.path_id) != me())) p1 = ataca_bfs(mapa, u.pos, u.health);
          else p1 = func_bfs(mapa, u.pos);
